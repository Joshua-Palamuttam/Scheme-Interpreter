...
(load "C:/Users/Administrator/Desktop/CS/Junior/PLC/Scheme-a-thon Assignments/Assignment 13/chez-init.ss")
...
(trace-define rib-find-position 
    (lambda (sym syms)
        (if (member sym syms)
...
(define apply-env-ref 
    (lambda (env sym)
        (if (null? env)
            (common-fail)                                  ;; Fail, the sym was not found
        (begin
            (let ([syms (caar env)]
              [vals (cdar env)]
              [env (cdr env)])
...
(define *prim-proc-names* '(+ - * / add1 sub1 cons = list car cdr caar cddr cadr cdar cadar
                            ...
                            map apply vector vector-set! vector-ref member quotient list-tail eqv?))
