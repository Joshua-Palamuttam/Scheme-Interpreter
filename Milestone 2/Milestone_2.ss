...
; (load "chez-init.ss") 
           ...
           [(eqv? (1st datum) 'set!)
              (if (< (length datum) 3)
                  ...
                  (if (> (length datum) 3)
                      ...
                      (set!-exp (syntax-expand (parse-exp (2nd datum))) (syntax-expand (parse-exp (3rd datum))))))]
                      ...
                      (if (not (proper-list? (2nd datum)))
                          ...
                          (if (not (andmap proper-list? (2nd datum)))
                              ...
                              (if (not (andmap (lambda (x) (equal? (length x) 2)) (2nd datum)))
                                  ...
                                  (if (not (andmap (lambda (x) (symbol? (car x))) (2nd datum)))
                                      (eopl:error 'parse-exp "Error in parse-exp: decls: first members must be symbols: ~s" (2nd datum))
                                      (letrec-exp (map car (2nd datum)) (map (lambda (x) (syntax-expand (parse-exp (cadr x)))) (2nd datum)) (map (lambda (y) (syntax-expand (parse-exp y))) (cddr datum)))))))))]   
          [(eqv? (1st datum) 'cond)
    ...
    (define (syntax-expand exp)
        (cases expression exp
            ...
            [letrec-exp (syms vals body) exp]
            [named-let-exp (proc-id syms vals body) exp]
    ...
    ; (trace-define (expand-while-exp cond bodies)
      ...
      (lambda (proc-value args)
        (cases proc-val proc-value
          ...
          [variable-args-closure (id bodies env)
            (let ([extended-env (extend-env (list id) (list args) env)])
                (eval-bodies bodies extended-env))]
          [improper-args-closure (id bodies env)
            (let ([extended-env (extend-env (improper->proper id) (group-args id args) env)])
                (eval-bodies bodies extended-env))]
    ...
    (define *prim-proc-names* '(+ - * / add1 sub1 cons = list car cdr caar cddr cadr cdar cadar
                                ...
                                map apply vector vector-set! vector-ref member quotient))
    ...
    (define apply-prim-proc  
      (lambda (prim-proc args)
        (case prim-proc
          ...
          [(vector-set!) (vector-set! (1st args) (2nd args) (3rd args))] ;; hmmmmmm
