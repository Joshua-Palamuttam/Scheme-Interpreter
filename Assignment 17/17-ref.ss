...
; (load "C:/Users/baileyrj/Documents/Rose-Hulman/CS/PLC 304/Scheme/17-test-code.ss")
    ...
    (define valid-L-param?
        (lambda (x)
            (or (symbol? x) (and (pair? x) (eqv? (car x) 'ref)))))
    ...
    (define-datatype expression expression?
        ...
        [lambda-with-refs-exp
         (id (list-of valid-L-param?))
         (body (list-of expression?))]
                    ...
                    (if (< (length datum) 3)                    
                        ...
                        (if (not (proper-list? (2nd datum))) 
                            (if (special-all-syms (2nd datum))
                                ...
                                (eopl:error 'parse-exp "Error in parse-exp: Special all syms check - lambda argument list: formals must be symbols: ~s" (2nd datum)))
                            (if (andmap (lambda (x) (and (symbol? x) (not (pair? x)))) (2nd datum))
                                (lambda-exp (2nd datum) (map (lambda (y) (syntax-expand (parse-exp y))) (cddr datum)))
                                (if (andmap (lambda (x) (or (symbol? x) (and (pair? x) (eqv? (car x) 'ref)))) (2nd datum))
                                    (lambda-with-refs-exp (2nd datum) (map (lambda (y) (syntax-expand (parse-exp y))) (cddr datum)))
    ...
    (define (syntax-expand exp)
        (cases expression exp
            ...
            [lambda-with-refs-exp (id body) exp]
    ...
    (define eval-exp  
      (lambda (exp env)
        (cases expression exp
          ...
          [lambda-exp (id body)
            (closure id body env)]
          [lambda-with-refs-exp (id body)
            (closure-with-refs id body env)]
          [lambda-no-paren-exp (id body) 
          ...
          [set!-exp (id value) 
                (set-ref! 
                    (apply-env-ref env id (lambda () (apply-env-ref global-env id common-fail)))
                    (eval-exp value env))]
          [set-car!-exp (id value)
                (apply-prim-proc 'set-car! (list (apply-env env id identity-proc (apply-env global-env id identity-proc common-fail)) (apply (lambda (x) (eval-exp x env)) (list value))))]
          [set-cdr!-exp (id value)
                (apply-prim-proc 'set-cdr! (list (apply-env env id identity-proc (apply-env global-env id identity-proc common-fail)) (apply (lambda (x) (eval-exp x env)) (list value))))]
            ...
            (let ([proc-value (eval-exp rator env)]
                  [args (eval-rands rands env)])
                    ...
                    (cases proc-val proc-value
                        [closure-with-refs (ids bodies environment)
                            ; (let ([checked (closure-check-refs ids rands env '() '() '())])
                                    ; (apply-proc (closure-with-refs (car checked) bodies env) (cadr checked))
                                    ; (update-refs (caddr checked) env environment))]
                            (apply-proc (closure-with-refs ids bodies env) rands)]
                        [else (apply-proc proc-value args)]))]

        [else (error 'eval-exp "Bad abstract syntax: ~a" exp)])))
    
    ; evaluate the list of operands, putting results into a list

    (define (closure-check-refs ids args env new-ids new-args refs)
        (cond
            [(null? ids) (list new-ids new-args refs)]
            [(symbol? (car ids)) (closure-check-refs (cdr ids) (cdr args) env (cons (car ids) new-ids) (cons (eval-exp (car args) env) new-args) refs)]
            [(eqv? (caar ids) 'ref) (closure-check-refs (cdr ids) (cdr args) env (cons (cadar ids) new-ids) (cons (eval-exp (car args) env) new-args) (cons (list (cadar ids) (cadar args)) refs))]
            [else (eopl:error 'closure-check-refs "something really wrong")]))

    (define update-refs
        (lambda (refs env)              ;; refs: '((var references-this) (var2 references-this))
            (for-each 
                (lambda (x) (set-ref! 
                                (apply-env-ref (cdr env) (cadr x) (lambda () (apply-env-ref global-env (cadr x) common-fail)))
                                (apply-env env (car x) identity-proc (lambda () (apply-env global-env (car x) identity-proc common-fail))))) refs)))
...
;      (env list?)]
    ...
    (define apply-env-ref 
        (lambda (env sym fail)
            ...
            (begin
              ...
              (let ([pos (rib-find-position sym syms)])
                (if (number? pos)
                  (refer vals pos)                   ;; Succeed, we found the sym
                  (apply-env-ref env sym (lambda () (apply-env-ref global-env sym common-fail))))))))))
    ...
    (define in-order-map
        (lambda (ls proc)
            (if (null? ls)
                '()
                (let ([evals (p (car ls))])
                    (cons (evals) (in-order-map (cdr ls) proc))))))
    ...
    (define apply-proc  
      (lambda (proc-value args)
        (cases proc-val proc-value
          ...
          [closure (ids bodies env) 
            ; (let ([checked (check-refs ids args env '() '())])
                (let ([extended-env (extend-env ids (list->vector args) env)])
                    (eval-bodies bodies extended-env))]
          [closure-with-refs (ids bodies environment)
            (let ([checked (closure-check-refs ids args environment '() '() '())])
                (let ([extended-env (extend-env (car checked) (list->vector (cadr checked)) environment)])
                    (eval-bodies bodies extended-env)
                    (update-refs (caddr checked) extended-env)))]
    ...
    ; (trace-define (check-refs ids args env new-ids new-args)
    ;         (cond
    ;             [(null? ids) (list new-ids new-args)]
    ;             [(symbol? (car ids)) (check-refs (cdr ids) (cdr args) env (cons (car ids) new-ids) (cons (car args) new-args))]
    ;             [(eqv? (caar ids) 'ref) (check-refs (cdr ids) (cdr args) env (cons (cadr ids) new-ids) (cons (apply-env-ref env (car args)) new-args))]
    ;             [else (eopl:error 'check-refs "something really wrong")]))
